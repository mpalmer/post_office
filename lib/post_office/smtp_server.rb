...
module PostOffice
  ...
  class SMTPServer < GenericServer
    attr_accessor :client_data, :messages

    # Create new server listening on port 25
    def initialize(port, logger: Logger.new("/dev/null"))
      self.client_data = Hash.new
      self.messages = []
      super(port: port, logger: logger)
    end
    ...
    def append_data(client, full_data)
      if full_data.gsub(/[\r\n]/,"") == "."
        ...
        self.messages << { from: get_client_data(client, :from).to_s, to: get_client_data(client, :to).to_s, data: get_client_data(client, :data).to_s }
        respond(client, 250)
        @logger.info "Received mail from #{get_client_data(client, :from).to_s} with recipient #{get_client_data(client, :to).to_s}"
      else
